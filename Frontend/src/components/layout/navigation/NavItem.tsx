...
// types
import type { NavItemType, RootStateProps } from './NavGroup';
import type {LinkTargetType} from "@tcpos/backoffice-components";
...
const NavItem = ({ item, level }: Props) => {
    ...
    let itemTarget: LinkTargetType = '_self';
    if (item.target) {
        itemTarget = '_blank';
    }
    ...
    useEffect(() => {
        if (pathname && pathname.includes('product-details')) {
            if (item.url && item.url.includes('product-details')) {
                dispatch(activeItem({ openItem: item.id ?  [item.id] : [] }));
            }
        }
